...
Given a new user attempts to sign up without providing all required information, then an error message is displayed, highlighting the missing fields.

Given a new user successfully signs up, then they are automatically logged in and redirected to their profile setup or a dashboard.
...
Given a musician user updates their instruments, genres, or location, when they save their profile, then these changes are successfully updated and used for match-making and search functionality.

Given a musician user has saved their profile, then their selected instruments, genres, and location are displayed on their public profile.
